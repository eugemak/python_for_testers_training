...
class TestAddGroup(unittest.TestCase):
    ...
    def test_add_user(self):
        ...
        self.init_new_user(wd)
        self.fill_new_user_fields(wd, AddUser(firstname="Carlos", middlename="Карлович", lastname="Sainz",
                                              title="Driver", company="McLaren F1 Team", home_phone="000-000-00"))
        self.save_user(wd)
        self.logout(wd)

    def save_user(self, wd):
        wd.find_element_by_xpath("(//input[@name='submit'])[2]").click()

    def init_new_user(self, wd):
        wd.find_element_by_link_text("add new").click()

    def fill_new_user_fields(self, wd, add_user):
